...
export default function AdminDashboard({ onNewOrderToast }) {
  ...
  const [confirmPaymentModal, setConfirmPaymentModal] = useState({ isOpen: false, orderIds: [], table: null });
  ...
  const handleMarkTablePaid = async (orderIds) => {
    try {
      ...
      // Fetch fresh orders and clear modal/tab
      fetchOrders();
      setSelectedBillingTable(null);
      setConfirmPaymentModal({ isOpen: false, orderIds: [], table: null });
    } catch (err) {
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (order.items && order.items.length > 0) {
                            // Find the longest prep time among ordered items, forcing integer
                            const maxPrepTime = Math.max(...order.items.map(item => parseInt(item.product?.prep_time_minutes || 5, 10)));
                            
                            // Calculate total quantity of items in this order, forcing integer
                            const totalQty = order.items.reduce((sum, item) => sum + parseInt(item.quantity || 1, 10), 0);
                            
                            // Smart Calculation: Base time is maxPrepTime, add 2 minutes for every additional item
                            finalPrepTime = maxPrepTime + Math.max(0, (totalQty - 1) * 2);
                          }
                          handleUpdateStatus(order.id, 'Preparing', finalPrepTime);
                        }}
                ...
                <>                  {/* Selected Table Summary */}
                  {selectedBillingTable && activeTables.includes(selectedBillingTable) && (
                    <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col lg:flex-row gap-8">
                      {/* Items List */}
                      <div className="flex-1 printable-bill font-sans">
                        {/* Print Header (Visible only when printing) */}
                        <div className="hidden print:flex flex-col items-center justify-center border-b-2 border-black pb-6 mb-6">
                          <h1 className="text-4xl font-black tracking-tight text-black uppercase">Menu Digital</h1>
                          <p className="text-gray-500 font-medium mt-1">{t('bill_for', { table: localizeTable(selectedBillingTable) })}</p>
                          <div className="text-xs text-gray-400 mt-2">{new Date().toLocaleString()}</div>
                        </div>

                        {/* Screen Header (Hidden when printing) */}
                        <div className="flex print:hidden items-center justify-between mb-6 border-b border-slate-800 pb-4">
                      ...
                      <div className="w-full lg:w-80 flex flex-col gap-4 hide-on-print">
                        ...
                        <button
                          onClick={() => {
                            ...
                            setConfirmPaymentModal({ isOpen: true, orderIds: summary.orderIds, table: selectedBillingTable });
                          }}
      ...
      {confirmPaymentModal.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div 
            ...
            onClick={() => setConfirmPaymentModal({ isOpen: false, orderIds: [], table: null })}
          />
          <div className="relative bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl z-10 animate-in fade-in zoom-in-95 duration-200">
            <div className="text-center">
              ...
              <p className="text-slate-400 text-sm mb-6 leading-relaxed">
                {t('confirm_payment_desc', { count: confirmPaymentModal.orderIds.length, table: confirmPaymentModal.table?.replace('Table', t('table')) })}
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => setConfirmPaymentModal({ isOpen: false, orderIds: [], table: null })}
                  className="flex-1 py-3.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold rounded-xl transition-colors cursor-pointer"
                >
                  {t('cancel')}
                </button>
                <button
                  onClick={() => handleMarkTablePaid(confirmPaymentModal.orderIds)}
                  className="flex-1 py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-xl transition-colors shadow-[0_0_20px_rgba(16,185,129,0.2)] cursor-pointer"
                >
                  {t('confirm', 'Confirm')}
                </button>
              </div>
